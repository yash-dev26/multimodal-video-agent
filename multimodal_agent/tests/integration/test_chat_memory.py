...
def test_extract_video_clip_rejects_non_positive_duration():
    with pytest.raises(ValueError, match="start_time must be less than end_time"):
        extract_video_clip(video_path="irrelevant.mp4", start_time=10.0, end_time=5.0, output_path="out.mp4")

    with pytest.raises(ValueError, match="start_time must be less than end_time"):
        extract_video_clip(video_path="irrelevant.mp4", start_time=5.0, end_time=5.0, output_path="out.mp4")
